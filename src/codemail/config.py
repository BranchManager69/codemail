...
PASSWORD_FILE = Path(os.environ.get("CODEMAIL_PASSWORD_FILE", HOME / ".codex_mail_pass")).expanduser()

SMTP_HOST = os.environ.get("CODEMAIL_SMTP_HOST", "mail.branch.bet")
SMTP_PORT = int(os.environ.get("CODEMAIL_SMTP_PORT", "587"))

STATE_PATH = Path(os.environ.get("CODEMAIL_STATE_PATH", HOME / ".codex" / "task_mail_map.json")).expanduser()
LOG_PATH = Path(os.environ.get("CODEMAIL_LOG_PATH", HOME / ".codex" / "task-mail-runner.log")).expanduser()
SESSION_ROOT = Path(os.environ.get("CODEMAIL_SESSION_ROOT", HOME / ".codex" / "sessions")).expanduser()

REPLY_TO = os.environ.get("CODEMAIL_REPLY_TO", "tasks@branch.bet")
DEFAULT_FALLBACK_RECIPIENT = os.environ.get("CODEMAIL_FALLBACK_RECIPIENT", "branch@branch.bet")
...
def resolve_mail_password() -> str:
    ...
    raise RuntimeError("SMTP password not provided; set CODEMAIL_MAIL_PASS or CODEMAIL_PASSWORD_FILE")
